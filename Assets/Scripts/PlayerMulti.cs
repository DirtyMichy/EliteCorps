...
public class PlayerMulti : UnitObject
{
	...
	//0 == no Player, 1 = Player 1
	public int hp = 10;
	//Ship's max hit points
	public int currentHP;
	//Ship's current hit points
	public int ammo = 100, maxAmmo;
	//Ship's ammo and maxAmmo
	public int specialAmmo = 4;
	//Ship's special ammo

	private bool bulletRDY = true;
	//you can only shoot when your gun is rdy
	private bool specialRDY = true;
	//same goes for specialWeapons
	public bool wrap = false;
	//wrapping means you can fly off the screen and return on the other side
	...
	void OnTriggerEnter2D (Collider2D c)
	{
		...
		if (c.GetComponent<Bullet> ().powerUpType == 0) {
			...
			if (c.GetComponent<Bullet> ().powerUpType == 1) {
				ammo += 128;
			} else if (c.GetComponent<Bullet> ().powerUpType == 2) {
				specialAmmo += 3;
			} else if (c.GetComponent<Bullet> ().powerUpType == 3) {
				currentHP += 10;
				healthBar.fillAmount = 100f / hp * currentHP / 100f;
			}
		}
