...
public class Spaceship : MonoBehaviour
{
    ...
    public GameObject debris;          //If the object is a DrillingPit spawn a ruin at death

    public bool canShoot;                   //Can this ship fire?

    public string unitName = "Default";     //DrillingRig for example is named "DrillingRig"

    public int bulletsPerShot = 1;          //How many bullets are spawned per shot

    public GameObject shield;

    protected Transform[] shotPositions;    //Fire points on the ship
    protected Transform[] shotSpecialPositions;    //Second fire points on the ship
    ...
    IEnumerator Shield()
    {
        ...
        else
        {
            ...
            while (shieldSeconds > 0)
            {
                ...
                if (shieldSeconds == 4)
                    shield.GetComponentInChildren<SpriteRenderer>().color = new Color(1f, 1f, 1f, 0.7f);
                else
                if (shieldSeconds == 3)
                    shield.GetComponentInChildren<SpriteRenderer>().color = new Color(1f, 1f, 1f, 0.5f);
                else
                if (shieldSeconds == 2)
                    shield.GetComponentInChildren<SpriteRenderer>().color = new Color(1f, 1f, 1f, 0.35f);
                else
                if (shieldSeconds == 1)
