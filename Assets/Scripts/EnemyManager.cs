...
{
    public Vector2 min;                                //Viewport
    public Vector2 max;                                //Viewport

    //float OBJECTIVESPAWN = 20; //Only for endlessMode
    float campaingObjectiveSpawn = 30; //Is being resettet in StartSpawnCoroutines()
    ...
    {
        //Loop indefinitely
        while (Manager.current.currentMenu == -1)
        {
            ...
            if (Manager.current.gameMode == 1) //1 = CampaignMissions
            {
                missionSpawns();
                yield return new WaitForSeconds(6f);
                //Spawn extraObjective for campaign
                if (Manager.current.missionMode == 3)
                {
            ...
            if (Manager.current.gameMode == 2) //2 = Endless missions
            {
                EndlessMissions();
                yield return new WaitForSeconds(10f / playerAlive.Length);
            }

            if (Manager.current.gameMode == 3) //3 = Survival
            {
                Waves();
                yield return new WaitForSeconds(10f / playerAlive.Length);
            }
    ...
    public IEnumerator SpawnProps()
    {
        ...
        while (Manager.current.currentMenu == -1)
        {
            ...
            Instantiate(cloud, cloud.transform.position = new Vector2(Random.Range(min.x, max.x), 10f), cloud.transform.rotation);

            yield return new WaitForSeconds((float)rng);
    ...
    void missionSpawns()
    {
        ...
        if (Manager.current.currentMissionSelection < 5)
        {
            ...
            if (Manager.current.currentMissionSelection == 4)
            {
                ...
                if (rng == 0)    //6 rng enemies no flakships
                {
        ...
        if (Manager.current.currentMissionSelection > 4 && Manager.current.currentMissionSelection < 10)
        {
            ...
            if (Manager.current.currentMissionSelection == 5)//VIP
            {
                int rng = Random.Range(0, 2); //-1
                if (rng == 0) //7 enemies
                    for (int i = 0; i < 7; i++)
            ...
            if (Manager.current.currentMissionSelection == 6)//Survive
            {
                for (int i = 0; i < 7; i++)
                {
                    spawnPosition = -6f + 2f * i; //FullscreenSpawn
            ...
            if (Manager.current.currentMissionSelection == 7)//Rage
            {
            ...
            if (Manager.current.currentMissionSelection == 8)//Objective
            {
            ...
            if (Manager.current.currentMissionSelection == 9)//Boss
            {
                for (int i = 0; i < 5; i++)
                {
