...
public class Enemy : Spaceship
{
    ...
    public bool isObjective = false;    //If its not a regular enemy
    public bool isGroundUnit = false;   //Ships or buildings are groundUnits
    ...
    bool dying = false;                 //some enemies have a dyingtimer, if shot they would award multiple times the pointvalue 

    public RectTransform bossHealth;    //importan for bossHealthBars
    public GameObject powerUp;          //if this isn't empty, the enemy can drop a powerUp
    ...
    protected override void OnEnable()
    {
        ...
        if (unitName == "Cannon") //Setting hp of bossCannon
        {
    ...
    private void OnTriggerStay2D(Collider2D collision)
    {
        ...
        if (collision.GetComponent<PlayerMulti>())
    ...
    void OnTriggerEnter2D(Collider2D c)
    {
        ...
        else
        {
            if (currentHP > 0)
            {
                ...
                if (unitName == "Boss02" && damageModifier == 3)
                    currentHP -= obj.power * 2;
                else
                    currentHP -= obj.power;
            }
